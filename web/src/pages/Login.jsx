...
export default function Login() {
  ...
  // Estado da seleção de escola (coordenador multi-escola)
  const [escolaSelection, setEscolaSelection] = useState(null) // { escolas, cpf, password }

  const { user, login, selectEscola, loading } = useAuth()
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success === true) {
      navigate(from, { replace: true })
    } else if (result.success === 'escola_selection') {
      setEscolaSelection({ escolas: result.escolas, cpf: result.cpf, password: result.password })
    } else {
  ...
  const handleSelectEscola = async (escolaId) => {
    setError('')
    setSubmitting(true)
    const result = await selectEscola(escolaSelection.cpf, escolaSelection.password, escolaId)
    if (result.success) {
      navigate(from, { replace: true })
    } else {
      setError(result.error || 'Erro ao selecionar escola.')
    }
    setSubmitting(false)
  }
  ...
  // ── Etapa 2: seleção de escola ──────────────────────────────────────────
  if (escolaSelection) {
    return (
      <div className="relative min-h-screen flex items-center justify-center p-6 overflow-hidden">
        <button
          onClick={() => { setEscolaSelection(null); setError('') }}
          className="absolute top-6 left-6 z-10 inline-flex items-center gap-2 px-4 py-2 rounded-[12px] border border-white/30 bg-white/20 text-white text-[0.9rem] font-semibold transition-colors hover:bg-white/30"
        >
          <ArrowLeft size={20} />
          Voltar
        </button>
        ...
        <div className="relative z-10 w-full max-w-[420px] bg-[rgba(255,255,255,0.98)] rounded-[24px] p-10 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.25)] backdrop-blur-[12px]">
          <header className="text-center mb-8">
            <img
              ...
              className="block h-[100px] w-auto max-w-[240px] object-contain mb-4 mx-auto"
            />
            <h2 className="font-['Plus_Jakarta_Sans',system-ui,sans-serif] text-[1.25rem] font-bold text-[#042f2e] mb-1">
              Selecione a escola
            </h2>
            <p className="text-sm text-[#64748b]">
              Você está vinculado a mais de uma escola. Escolha em qual deseja entrar.
            </p>
          </header>

          {error && (
            <div className="mb-4 px-4 py-3 bg-[#fef2f2] border border-[#fecaca] text-[#b91c1c] rounded-[12px] text-sm" role="alert">
              {error}
            </div>
          )}

          <div className="flex flex-col gap-3">
            {escolaSelection.escolas.map((escola) => (
              <button
                key={escola.id}
                onClick={() => handleSelectEscola(escola.id)}
                disabled={submitting}
                className="flex items-center gap-3 w-full px-4 py-4 rounded-[12px] border-2 border-[#e2e8f0] bg-white text-left transition-all hover:border-[#0d9488] hover:bg-[#f0fdfa] disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <span className="flex-shrink-0 w-9 h-9 rounded-full bg-[#ccfbf1] flex items-center justify-center">
                  <Building2 size={18} className="text-[#0d9488]" />
                </span>
                <span className="font-semibold text-[#042f2e] text-sm leading-tight">
                  {escola.nome_escola}
                </span>
              </button>
            ))}
