...
async def list_modalidades(
    ...
    async with conn.cursor() as cur:
        await cur.execute(
            "SELECT id, nome, descricao, categoria, requisitos, ativa, created_at, updated_at FROM modalidades ORDER BY nome"
        )
...
async def get_modalidade(
    ...
    async with conn.cursor() as cur:
        await cur.execute(
            "SELECT id, nome, descricao, categoria, requisitos, ativa, created_at, updated_at FROM modalidades WHERE id = %s",
            (modalidade_id,),
        )
...
async def create_modalidade(
    ...
    async with conn.cursor() as cur:
        ...
        await cur.execute(
            """
            INSERT INTO modalidades (id, nome, descricao, categoria, requisitos, ativa)
            VALUES (%s, %s, %s, %s, %s, %s)
            RETURNING id, nome, descricao, categoria, requisitos, ativa, created_at, updated_at
            """,
            (
                id_final,
                data.nome.strip(),
                (data.descricao or "").strip(),
                data.categoria.upper(),
...
async def update_modalidade(
    ...
    async with conn.cursor() as cur:
        await cur.execute(
            "SELECT id, nome, descricao, categoria, requisitos, ativa FROM modalidades WHERE id = %s",
            (modalidade_id,),
        )
